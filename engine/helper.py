...
def extract_yt_term(command):
    ...
    pattern=r'play\s+(.*?)\s+on\s+youtube' # use re.search to find the match in the commanf]d
    match=re.search(pattern,command,re.IGNORECASE)
    # if a mtch is found return the extended song nane otherwise return none
...
# input_string = "make a phone call to kamlesh"
